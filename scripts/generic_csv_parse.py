...
def parse():
    ...
    for row in CsvFile:
        try:
            ...
            idlist= row[29].split('_')
            _id= int(idlist[3]+idlist[1])
            lon = row[3]
            lat=row[4]
            sci_name = Species.objects.get(common_name__icontains=row[2]).scientific_name
            ...
            height= int(row[6])
            condition='fair'
            of.writerow([_id,lat,lon,genus),str(species),cultivar,gender,address,'','','','','','','',diameter,height,'',condition,'','','',])
        except:            
            print "!",
            errFile.writerow(row)
